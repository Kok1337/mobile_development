...
class GetAllSubForestryUseCase @Inject constructor(
    private val subForestryRepo: SubForestryRepo,
) {
    suspend fun invoke(localForestry: LocalForestry): List<SubForestry> = subForestryRepo.getAllSubForestryByLocalForestryId(localForestry.id)
}